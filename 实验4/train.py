...
def measure_comm_time(model, criterion, optimizer, images, labels, device):
    optimizer.zero_grad()
...
def train_model(params, rank, local_rank, world_size):
    ...
    if rank == 0 and writer is not None:
        writer = SummaryWriter(
            f"runs/ws{world_size}_{params['model_name']}_bs{params['batch_size']}_gloo"
        )
    ...
    for epoch in range(params["epochs"]):
        ...
        for images, labels in trainloader:
            ...
            if params["measure_comm"] and len(sample_batches) < params["comm_batches"]:
        ...
        if params["measure_comm"] and world_size > 1 and sample_batches:
            total_iter_time, compute_iter_time, comm_time, comm_ratio = measure_comm_time_avg(
    ...
    if rank == 0:
        print(f"Model: {params['model_name']} | Params: {param_count}")
        print(f"Total Train Time: {total_train_time:.4f}s")
