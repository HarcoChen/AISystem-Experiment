...
def test(model, img, epsilon):
    ...
    # target-stole
    adv_label = 824 * torch.ones_like(target)
...
def main():
    ...
    image_np = load_image(
        shape=(224, 224), data_format="channels_first", abs_path=True, fpath=image_path
    )
    images_var = Variable(torch.unsqueeze(torch.tensor(image_np), dim=0).float()).to(
        device
    )
