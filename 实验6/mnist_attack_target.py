...
def test_target(model, device, test_loader, epsilon):
    ...
    for data, target in tqdm(test_loader):
        ...
        loss = F.nll_loss(output, adv_label)
        ...
        if (final_pred.item() == adv_label.item()) and (
            target.item() != adv_label.item()
        ):
            suc += 1
...
def main():
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig("results/mnist_fgsm_target.png")
